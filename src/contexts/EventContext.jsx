...
export const EventProvider = ({ children }) => {
    ...
    try {
      ...
      const sampleEventTypes = [
        ...
        },
        {
          ...
          description: "Professional business events, conferences, and meetings",
          organizerId
        },
        {
          name: "Birthday Party",
      ...
      const sampleEventPackages = [
        {
          ...
          description: "Complete wedding planning service including venue selection, catering, decoration, and day-of coordination.",
          price: 5000,
        ...
        },
        {
          ...
          description: "Professional conference planning including venue, AV equipment, catering, and registration management.",
          price: 3500,
        ...
        },
        {
          ...
          description: "Fun-filled birthday party planning with themes, activities, decorations, and catering.",
          price: 800,
          location: "Party Zone",
          organizerId,
          status: "Active",
  ...
  const getEventTypeById = async (id) => {
    try {
      ...
      const typeFromState = Array.isArray(eventTypes) ? 
        eventTypes.find(type => type.id === id) : null;
  ...
  const getEventTypesByOrganizer = (organizerId) => {
    ...
    return eventTypes.filter((eventType) => eventType.organizerId === organizerId);
  };
  ...
  const getEventPackageById = async (id) => {
    try {
      ...
      const packageFromState = Array.isArray(eventPackages) ? 
        eventPackages.find(pkg => pkg.id === id) : null;
  ...
  const getEventPackagesByType = (eventTypeId) => {
    ...
    return eventPackages.filter((eventPackage) => eventPackage.eventTypeId === eventTypeId);
  };

  const getEventPackagesByOrganizer = (organizerId) => {
    if (!organizerId || !Array.isArray(eventPackages)) return [];
    return eventPackages.filter((eventPackage) => eventPackage.organizerId === organizerId);
  };
  ...
  const createEventRequest = async (requestData) => {
    try {
      ...
      if (!requestData.packageId || !requestData.organizerId || !requestData.eventDate) {
      ...
      const createdRequest = {
        ...
        eventDate: new Date(requestData.eventDate).toISOString()
      };
      ...
      if (request) {
        try {
          ...
          if (organizerDoc.exists()) {
            ...
            const newRating = ((currentRating * currentReviewCount) + feedbackData.rating) / (currentReviewCount + 1);
  ...
  const getEventRequestById = async (id) => {
    try {
      ...
      const requestFromState = Array.isArray(eventRequests) ? 
        eventRequests.find(req => req.id === id) : null;
  ...
  const getEventRequestsByRequester = (requesterId) => {
    ...
    return eventRequests.filter((request) => request.requesterId === requesterId);
  };

  const getEventRequestsByOrganizer = (organizerId) => {
    if (!organizerId || !Array.isArray(eventRequests)) return [];
    return eventRequests.filter((request) => request.organizerId === organizerId);
  };
